...
function PostsDAO(db) {
    ...
    this.incrementLikes = function(permalink, comment_ordinal, callback) {
        ...
        */
        ...
        var postSelector = {'permalink' : permalink};
        console.log('=== postSelector: ' + JSON.stringify(postSelector, null, 2));
        var commentSelector = {};
        commentSelector['comments.' + comment_ordinal + '.num_likes'] = 1;
        console.log('=== commentSelector: ' + JSON.stringify(commentSelector, null, 2));
        posts.update(postSelector, {'$inc': commentSelector}, function(err, numModified) {
           if (err) return callback(err, null);
            callback(err, numModified);
        });
    }
